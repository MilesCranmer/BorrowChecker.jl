@testitem "Experimental @borrow_checker" tags = [:unstable] begin
    ...
    @testset "callee recursion" begin
        f_bad(; x, y) = (push!(x, 1); push!(y, 1); x .+ y)

        function foo_unchecked()
            x = [1, 2, 3]
            y = x
            return sum(f_bad(; x=x, y=y))
        end

        @borrow_checker g() = foo_unchecked()

        @test_throws BorrowCheckError g()
    end

    @testset "callee_check_roots policy" begin
        module _BCRecA
        using BorrowChecker.Experimental: BorrowCheckError, check_signature
        ...
        g_calls_bar() = _BCRecB.bar_bad()
        end # module _BCRecA

        tt_foo = Tuple{typeof(_BCRecA.g_calls_foo)}
        tt_bar = Tuple{typeof(_BCRecA.g_calls_bar)}

        cfg_none = BorrowChecker.Experimental.Config(; callee_check_roots=:none, max_callee_depth=1)
        @test BorrowChecker.Experimental.check_signature(tt_foo; cfg=cfg_none) == true
        @test BorrowChecker.Experimental.check_signature(tt_bar; cfg=cfg_none) == true

        cfg_same = BorrowChecker.Experimental.Config(; callee_check_roots=:same_package, max_callee_depth=1)
        @test_throws BorrowCheckError BorrowChecker.Experimental.check_signature(tt_foo; cfg=cfg_same)
        @test BorrowChecker.Experimental.check_signature(tt_bar; cfg=cfg_same) == true

        cfg_roots_b = BorrowChecker.Experimental.Config(;
            callee_check_roots=[_BCRecA._BCRecB],
            max_callee_depth=1,
        )
        @test_throws BorrowCheckError BorrowChecker.Experimental.check_signature(tt_bar; cfg=cfg_roots_b)

        cfg_all = BorrowChecker.Experimental.Config(; callee_check_roots=:all, max_callee_depth=1)
        @test_throws BorrowCheckError BorrowChecker.Experimental.check_signature(tt_foo; cfg=cfg_all)
        @test_throws BorrowCheckError BorrowChecker.Experimental.check_signature(tt_bar; cfg=cfg_all)
    end
end
