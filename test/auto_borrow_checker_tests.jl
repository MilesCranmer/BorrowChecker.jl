@testitem "Auto @safe" tags = [:auto] begin
    using TestItems
    using BorrowChecker
    using LinearAlgebra
    ...
    @testset "macro signature parsing: varargs" begin
        @safe function _bc_varargs_signature(xs...)
            return 0
        end
    ...
    @testset "macro signature parsing: default args" begin
        @safe function _bc_default_arg_signature(x=1)
            return x + 1
        end
    ...
    @testset "macro signature parsing: keyword-only signature" begin
        @safe function _bc_keyword_only_signature(; x, y)
            return x + y
        end
    ...
    @testset "macro signature parsing: anonymous typed arg" begin
        @safe function _bc_anon_typed_arg_signature(x, ::Type{T}=Int) where {T}
            return T
        end
    ...
    @testset "macro signature parsing: destructuring arg" begin
        @safe function _bc_destructure_signature((a, b))
            return a + b
        end
    ...
    @testset "macro signature parsing: where + return type" begin
        @safe function _bc_where_ret_signature(x::T)::T where {T}
            return x
        end
    ...
    @testset "boxed captured variable: getproperty field type refinement" begin
        ...
        @safe function _bc_boxed_getproperty_dim(x::_BCBoxedField)
            g = () -> getfield(x, :n)
    ...
    @testset "boxed captured variable: broadcast materialize should not consume" begin
        ...
        @safe function _bc_boxed_broadcast_ok(x::_BCBoxedBroadcast)
            g = () -> getfield(x, :n)
            x = fakewrite(x)  # capture + assign forces Core.Box lowering
    ...
    @testset "known failure: Array{Int,l}(x) with value l" begin
        @safe scope = :function function _bc_array_value_dim_ctor(x)
            l = 1
    ...
    @testset "@safe assignment instrumentation: store should not create fresh origins" begin
        ...
        @safe function _bc_projection_store_ok(s::_BCProjectionS)
    ...
    @testset "@safe known effects: eachindex should not consume/escape" begin
        @safe function _bc_eachindex_ok(refs, constants)
    ...
    @testset "@safe known effects: copy should not consume" begin
        @safe function _bc_copy_call_ok(x)
            y = copy(x)
            return (x, y)
        end
    ...
    @testset "@safe debug logging" begin
        using Test

        Base.@noinline _bc_dbg_localfun(x) = x

        @safe debug = true debug_callee_depth = 1 function _bc_dbg_fail(n::Int)
            x = zeros(Float64, n)
        ...
        @safe debug = true debug_callee_depth = 0 function _bc_dbg_depth0(n::Int)
            x = zeros(Float64, n)
            x2 = _bc_dbg_localfun(x)
            y = x2
            x2[1] = 0.0
        ...
        @safe debug = true debug_callee_depth = 0 function _bc_dbg_ok(x)
            return x + 1
        end
        ...
        @test count(
            lr ->
                lr.level == Base.CoreLogging.Warn && occursin(
                    "BorrowChecker.@safe debug enabled; writing JSONL debug log to",
                    lr.message,
                ),
            logs,
        ) == 1
        ...
        @safe debug = true optimize_until = "definitely_invalid_pass" function _bc_dbg_bad_opt(
            x
        )
            return x + 1
        end
        ...
        @safe debug = true scope = :function function _bc_dbg_summary_exception(x)
    ...
    @testset "callable structs: good/bad for mutable/immutable" begin
        ...
        @safe function _bc_mut_callable_bad()
            f = _BCMutCallable(0)
            g = f
            f()
            return g.n
        ...
        @safe function _bc_mut_callable_good()
            f = _BCMutCallable(0)
            g = f
            f(Val(:peek))
            return g.n
        ...
        @safe function _bc_imm_callable_bad()
            f = _BCImmCallable([0])
            g = f
            f()
            return g.v
        ...
        @safe function _bc_imm_callable_good()
            f = _BCImmCallable([0])
            g = f
            f(Val(:peek))
            return g.v
    ...
    @testset "@generated callee mutation is detected" begin
        ...
        @safe function _bc_generated_mutation_bad()
            x = [1, 2, 3]
            y = x
            _bc_gen_my_push!(x, 4)
            return y
    ...
    @testset "lambda arglist: single argument" begin
        @safe function _bc_lambda_arglist_symbol()
            f = x -> x + 1
            return f(1)
        end
    ...
    @testset "kwcall unknown-call consume expands to keyword values" begin
        ...
        @safe function _bc_kwcall_unknown_consume_should_error(vf)
            x = [1, 2, 3]
            y = x
            g = only(vf)
            g(; x=x, y=y)
    ...
    @testset "@safe scope=:module recursive callees" begin
        ...
        @safe function _bc_scope_outer_norec_ok()
            return _bc_scope_inner_bad()
        end
        @test _bc_scope_outer_norec_ok() == [0, 2, 3]

        @safe scope = :module function _bc_scope_outer_rec_bad()
            return _bc_scope_inner_bad()
        end
    ...
    @testset "modules are not owned (avoid spurious consumes)" begin
        @safe function _bc_module_not_owned()
            m = Base
    ...
    @testset "isa is pure (does not consume)" begin
        @safe function _bc_isa_does_not_consume()
            x = [1, 2, 3]
            y = fakewrite(x)
            if y isa Vector{Int}
                y[1] = 0
    ...
    @testset "scope=:all does not crash on PhiCNode" begin
        @safe scope = :all _bc_scope_all_sin(x) = sin(x)
        err = try
    ...
    @testset "Core.throw_inexacterror does not BorrowCheckError" begin
        ...
        @safe scope = :user _bc_inexact_int64(x::UInt64) = Int64(x)
    ...
    @testset "Pointer intrinsics + known issues" begin
        @safe function _bc_pointerset_ok()
            A = [1, 2, 3]
            p = pointer(A)
            unsafe_store!(p, 99, 1)
            return nothing
        ...
        @safe function _bc_pointer_unsafe_store_regression()
            A = [1, 2, 3]
            B = A
            p = pointer(A)
            unsafe_store!(p, 99, 1)
        ...
        @safe function _bc_pointerset_alias_bad()
            A = [1, 2, 3]
            p = pointer(A)
            q = p
            unsafe_store!(p, 99, 1)
        ...
        @safe function _bc_reinterpret_write_bad()
            A = Int32[1, 2, 3, 4]
            B = A
    ...
    @testset "read-only Base foreigncalls" begin
        ...
        @testset "jl_type_hash" begin
            @safe scope = :all function _bc_hash_type_alias_ok(T)
                S = T
                hash(T)
                return T === S
            end
        ...
        @testset "jl_type_unionall" begin
            @safe scope = :all function _bc_unionall_typearg_alias_ok(t)
                u = t
        ...
        @testset "jl_eqtable_get" begin
            @safe scope = :all function _bc_iddict_get_alias_ok(d, k)
                d2 = d
                get(d, k, nothing)
                return d === d2
            end
        ...
        @testset "jl_eqtable_nextind (via iterate)" begin
            @safe scope = :all function _bc_iddict_iterate_alias_ok(d)
                d2 = d
                iterate(d)
                return d === d2
            end
        ...
        @testset "jl_get_fieldtypes" begin
            @safe scope = :all function _bc_fieldtypes_alias_ok(T)
                S = T
                fieldtypes(T)
                return T === S
            end
        ...
        @testset "jl_field_index" begin
            @safe scope = :all function _bc_fieldindex_alias_ok(T)
                S = T
        ...
        @testset "jl_gc_new_weakref_th" begin
            @safe scope = :all function _bc_weakref_alias_ok(x)
                y = x
                WeakRef(x)
                return x === y
            end
        ...
        @testset "jl_value_ptr" begin
            @safe scope = :all function _bc_ptr_from_objref_alias_ok(x)
                y = x
    ...
    @testset "String arguments are not owned" begin
        @test !BorrowChecker.Auto.is_owned_type(String)
        @test !BorrowChecker.Auto.is_owned_type(SubString{String})
        @test !BorrowChecker.Auto.is_tracked_type(String)
        ...
        @test err === nothing || !(err isa BC.Auto.BorrowCheckError)
    end
